...
if(*query!='\0'){
    ...
    if(totalqterm==0){
        ...
        for(i=0;i<totalqterm;i++){
            ...
            if(pos<0){ /** jika term terdapat pada dokumen */
                ...
                /////////// DEBUG menampilkan isi ilbuf ///////////////////////
                // for (int i = 0; i < len; i++) {
                //     // printf("panjang ilbuf: %d\n", len);
                //     printf("%ld ", ilbuf[i]);
                // }
                ...
                for(t=0;t<fw;t++){
                    docno  = ilbuf[t*2];
                    freqwd = ilbuf[t*2+1];
                    // s_q_d  = log(N/fw+1)*log(freqwd+1);  // DISINI ERRORNYA 156
                    if (N != 0 && fw != 0 && freqwd != 0) {
                        s_q_d = log(N/fw + 1) * log(freqwd + 1);
                    } 
                    else {
                        // Handle error or set s_q_d to a default value
                        s_q_d = 0.0; // Default value (you can choose a suitable default)
                    }
    ...
    for(i=0;i<N;i++){
        if(i<HEAPSIZE){
            ...
            if(accumulator[i]>heap[0].ranked) { // DISINI ERRORNYA 181
